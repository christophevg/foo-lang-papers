...
\begin{tikzpicture}[
  ...
  \node (model) [class] {Model}
      child {node (module ) [class] {Module}
        child {node (strategy) [class,align=center,xshift=-9mm] {Execution\\Strategy}
          child {node(every) [class,xshift=-9mm] {Every}}
          child {node (when) [class,xshift=9mm] {When}}
        }
        child {node [class,xshift=9mm] {function}}
      };
