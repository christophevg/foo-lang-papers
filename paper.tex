...
Reusing common data allows elimination of for example the network address
within the scope of each individual algorithm. Analysis of the source code with
both algorithms implemented, shows that this reduces the memory footprint for
one node with 2 bytes from 27 bytes to 25 bytes. A ZigBee network address
indeed consists of 16 bits. So this results in a reduction of 7.4\%. Using
ZigBee's 64 bits unique addresses, this reduction can easily become 20.5\%.

Finally comparing the amount of code versus the size of the formal description
of IDS algorithms, can be an indication of the development effort. Implementing
both algorithms manually results in 637 lines of clean and documented C code.
The corresponding \NAME implementation results in 156 lines of DSL code.
