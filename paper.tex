...
imposes significant overhead on resource-constraint devices such as those in
the Internet of Things. Fusion of the algorithms, to optimize sharing of
resources, can alleviate this, yet, proves to be time-consuming, repetitive and
error-prone. To address this problem \NAME provides a framework for the
...
source code. A side-by-side comparison shows that generated code is smaller and
reduces message passing overhead, execution time and memory footprint in
comparison to a straightforward sequential implementation of the same
algorithms.
...
detection (ID) comes into play: an intrusion detection systems (IDS) monitors
...
\begin{algorithmic}[1]
  \Require{devices: global storage for device information}
  \Function{process\_message}{$msg$}
    ...
    \State \Call{send}{$device_y$, ``info''} \Comment{optionally exchange info} \label{alg:id-algo-pattern-send1}
  \EndFunction
  \State
...
ATMEGA1284p micro-controller and the Digi XBee S2 ZigBee module. A basic
library wrapped the technical calls to the hardware components in more easy to
use functions.
...
development effort. As explained in \ref{positioning}, \NAME focuses on the
reduction of the development effort and update cost, while preserving maximal
flexibility.
