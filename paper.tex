...
\begin{LaTeXdescription}
  ...
  \item[Flexibility] is a measure for the number of applications that can be
  deployed on a device without physical access.
...
to the \emph{Execution Strategy}. This structure encompasses the actual
...
In this paper we introduced \NAME, a framework consisting of a DSL, a source
code generator and accompanying software library. Using the DSL, intrusion
detection algorithms can be formally described in such way that, using
functional code fusion during source code generation, it is possible to
interpret their functional meaning and merge common functionality and shared
data. The software library hosts common functionality, such as network message
parsing, sent message aggregation, event scheduling,\dots as a framework for
...
generated intrusion detection systems. We see opportunities to allow
application to actively and dynamically respond to intrusions as they are
detected and avoid malicious payload to harm them.
