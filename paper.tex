...
\begin{tikzpicture}[
  ...
  \node (sm)   [representation,right=of dsl] {Semantic\\Model};
  \node (cm)   [representation,right=of sm]  {Code\\Model};
...
\tikzset{
  ...
  aggregate/.style={thick,draw=black,>=diamond,<-}
}

\begin{figure}[H]
\begin{tikzpicture}[
  edge from parent fork down,
  sibling distance=5mm, level distance=20mm, node distance=8mm
  ]
  \node (model) [class] {Model}
      child {node (module ) [class] {Module} edge from parent [aggregate]
        child {node (strategy) [class,align=center,xshift=-9mm] {Execution\\Strategy} edge from parent [aggregate]
        ...
        child {node [class,xshift=9mm] {function} edge from parent [aggregate]}
      };
  \node (constant) [class,right=of model] {Constant};
  \node (scope)    [class,left=of strategy] {Scope};
  
  \draw [thick,>=latex,->] (strategy) -- (scope);
  \draw [aggregate] (model) -- (constant);
\end{tikzpicture}
\caption{Partial Semantic Model Meta-Model}
